...
export const AdminConstructorPage = (): JSX.Element => {
    ...
    const [isOpenCreateSectionModal, setIsOpenCreateSectionModal] =
        useState<boolean>(false);
    const [isOpenCreateSubsectionModal, setIsOpenCreateSubsectionModal] =
        useState<boolean>(false);
    const [isOpenEditSectionModal, setIsOpenEditSectionModal] =
        useState<boolean>(false);
    const [isOpenEditSubsectionModal, setIsOpenEditSubsectionModal] =
        useState<boolean>(false);
    const [isOpenDeleteSectionModal, setIsOpenDeleteSectionModal] =
        useState<boolean>(false);
    const [isOpenDeleteSubsectionModal, setIsOpenDeleteSubsectionModal] =
        useState<boolean>(false);
    ...
    const [selectedSubsection, setSelectedSubsection] =
        useState<SubsectionInfo | null>(null);
    ...
    const updateSectionsFullData = () => {
        httpClient
            .axios()
            .get<SectionInfo[]>(config.endPoints.getAllSectionsTree)
    ...
    return (
        <div className="admin-constructor-page">
                            ...
                            {sectionInfo.subsections?.map((subsectionInfo) => (
                                        ...
                                        <Flex justify="letf" align="center">
                                            ...
                                            <Button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    setIsOpenDeleteSubsectionModal(
                                                        true
                                                    );
                                                }}
                                                danger
                                ...
                                >
                                    <Button
                                        ...
                                        onClick={() => {
                                            navigate(
                                                `${sectionInfo.id}/${subsectionInfo.id}`
                                            );
                                        }}
                                    >
                                        Управлять
                ...
                onOk={() => {
                    createSectionForm
                        ...
                        .then((fields) => {
                            httpClient
                                .axios()
                                .post(config.endPoints.createSection, {
                                    text: fields?.text,
                                })
                                .then(() => {
            ...
            >
                <Form form={createSectionForm} layout="vertical">
                    <Form.Item
                        name="text"
                        label="Название:"
                        required
                        rules={[
                            ...
                            },
                            {
                                ...
                                min: 5,
                                message:
                                    "Название: длина должна быть больше или равна 5 символам!",
                            },
                            {
                                pattern: /^[,.а-яА-ЯёЁ\s]+$/,
                                message: "Название: неверный формат поля!",
                            },
                        ]}
                    >
                        <Input
            ...
            <Modal
                        ...
                        createSubsectionForm
                            ...
                            .then((fields) => {
                                httpClient
                                    .axios()
                                    .post(
                                        config.endPoints.createSubsection.replace(
                                            "{sectionId}",
                                    ...
                                    .then(() => {
                                        notification.success({
                                            message:
                                                "Подраздел успешно создан!",
                                        });
                                        setIsOpenCreateSubsectionModal(false);
                                        createSubsectionForm.resetFields([
                                            "text",
                                        ]);
                                        setSelectedSection(null);
                                        updateSectionsFullData();
                ...
                cancelButtonProps={{ shape: "round", type: "text" }}
            >
                <Form form={createSubsectionForm} layout="vertical">
                    <Form.Item
                        name="text"
                        label="Название:"
                        required
            ...
            <Modal
                        ...
                        editSectionForm
                            ...
                            .then((fields) => {
                                httpClient
                                    .axios()
                                    .put(
                                        config.endPoints.editSection.replace(
                                            "{sectionId}",
                                            selectedSection?.id.toString()
                                        ),
                                        {
                ...
                cancelButtonProps={{ shape: "round", type: "text" }}
            >
                <Form form={editSectionForm} layout="vertical">
                    <Form.Item
                        name="text"
                        label="Название:"
                        required
                        initialValue={selectedSection?.text}
                        rules={[
                            {
                                required: true,
                                message: "Название: обязательное поле!",
                ...
                onOk={() => {
                    if (selectedSection) {
                        httpClient
                            .axios()
                            .delete(
                                config.endPoints.deleteSection.replace(
                                    "{sectionId}",
                                    selectedSection?.id.toString()
                                )
                            )
            ...
            <Modal
                        ...
                        editSubsectionForm
                            ...
                            .then((fields) => {
                                httpClient
                                    .axios()
                                    .put(
                                        config.endPoints.editSubsection
                                            .replace(
                                                "{sectionId}",
                                                selectedSection?.id.toString()
                                            )
                                            .replace(
                                                "{subsectionId}",
                                                selectedSubsection.id.toString()
                                            ),
                                        {
                                            text: fields?.text,
                                        }
                                    ...
                                    .then(() => {
                                        notification.success({
                                            message:
                                                "Подраздел успешно изменен!",
                                        });
                                        setIsOpenEditSubsectionModal(false);
                                        editSubsectionForm.resetFields([
                                            "text",
                                        ]);
                                        setSelectedSection(null);
                                        setSelectedSubsection(null);
                ...
                cancelButtonProps={{ shape: "round", type: "text" }}
            >
                <Form form={editSubsectionForm} layout="vertical">
                    <Form.Item
                        name="text"
                        label="Название:"
                        required
                        initialValue={selectedSubsection?.text}
                        rules={[
                            {
                                required: true,
                                message: "Название: обязательное поле!",
                ...
                onOk={() => {
                    if (selectedSection && selectedSubsection) {
                        httpClient
                            .axios()
                            .delete(
                                config.endPoints.deleteSubsection
                                    .replace(
                                        "{sectionId}",
                                        selectedSection?.id.toString()
                                    )
            ...
            >
                <Paragraph>
                    Вы действительно хотите удалить данный подраздел?
                </Paragraph>
            </Modal>
        </div>
    );
