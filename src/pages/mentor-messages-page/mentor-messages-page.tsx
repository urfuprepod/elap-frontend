...
export const MentorMessagesPage = (): JSX.Element => {
    ...
    const [messagesAgreementFileUrl, setMessagesAgreementFileUrl] =
        useState<string>("");
    const [isEditMessagesAgreementFileUrl, setIsEditMessagesAgreementFileUrl] =
        useState<boolean>(false);
    const [isOpenRequestModal, setIsOpenRequestModal] =
        useState<boolean>(false);
    const [isOpenCreateModal, setIsOpenCreateResponse] =
        useState<boolean>(false);
    ...
    const [createResponseForm] = Form.useForm<{
        response: string;
    ...
    const updateMentorMessagesData = () => {
        httpClient
            .axios()
            .get<Message[]>(config.endPoints.getAllMessages)
            .then((response) => {
                    ...
                    <Button
                        ...
                        onClick={() => {
                            ...
                            httpClient
                                .axios()
                                .get<UserInfo>(
                                    config.endPoints.getStudent.replace(
                                        "{userId}",
                        ...
                        <Button
                            ...
                            onClick={() => {
                                httpClient
                                    .axios()
                                    .post(
                                        config.endPoints.changeMessageStatus.replace(
                                            "{messageId}",
                                            record.id.toString()
                                        )
                                    )
    ...
    return (
        <div className="mentor-messages-page">
            ...
            >
                ...
                <Paragraph>
                    <Text strong>Файлы вопроса: </Text>
                </Paragraph>
                <FileList
                    link="messages"
                    files={
                        requestContent?.questionFiles
                            ? requestContent?.questionFiles
                            : []
                    }
                />
                ...
                onOk={() => {
                    if (requestContent) {
                        createResponseForm
                            .validateFields()
                            .then((fields) => {
                                const responseFiles = fields.filesList?.length
                                ...
                                httpClient
                                    .axios()
                                    .post(
                                        config.endPoints.createMessageResponse,
                                        formData
                                    )
                                    .then((response) => {
                                        notification.success({
                                        ...
                                        setIsOpenCreateResponse(false);
                                        createResponseForm.resetFields([
                                            "response",
                                            "filesList",
                                        ]);
                                        updateMentorMessagesData();
            ...
            >
                <Form form={createResponseForm} layout="vertical">
                    <Form.Item
                        name="response"
                        label="Ответ:"
                        required
                        rules={[
                            {
                                required: true,
                                message: "Ответ: обязательное поле!",
                            },
                            {
                                type: "string",
                                min: 10,
                                message:
                                    "Ответ: длина должна быть больше или равна 10 символам!",
                            },
                        ]}
                    >
                        <TextArea
