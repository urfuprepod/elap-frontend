...
export const MessagesPage = (): JSX.Element => {
    ...
    const [isOpenResponseModal, setIsOpenResponseModal] =
    ...
    const [isNewQuestionModal, setIsNewQuestionModal] =
        useState<boolean>(false);
    ...
    const updateMessageAgreementFile = () => {
        httpClient
            .axios()
            .get<string>(config.endPoints.getMessageAgreementFileUrl)
            .then((response) => {
    ...
    const showResponseModal = (message: Message) => {
        ...
        setResponseFiles(
            message.responseFiles?.length ? message.responseFiles : []
        );
            ...
            {authContext ? (
                <>
                    ...
                    <Paragraph>
                        ...
                        Перед тем, как отправить вопрос Вашему ментору,
                        ознакомьтесь с
                        <a
                            href={messagesAgreementFileUrl}
                            target="_blank"
                            rel="noreferrer"
                        >
                            {" "}
                        ...
                        модуля «Вопросы ментору».
                        <br />
                    </Paragraph>
                    <Paragraph className="signature" italic>
                        С уважением,
                        <br />
                        команда проекта «ЭЛАП»
                    </Paragraph>
                    <Divider />
                    <Table
                        columns={columns}
                    ...
                    >
                        ...
                        <FileList link="messages" files={responseFiles ? responseFiles : []} />
                    </Modal>
                    ...
                    <Modal
                        ...
                        onOk={() => {
                            newQuestionForm
                                .validateFields()
                                .then((fields) => {
                                    const newMessageFiles = fields.filesList
                                        ?.length
                    ...
                    >
                        <Form form={newQuestionForm} layout="vertical">
                            <Form.Item
                                name="theme"
                                label="Тема:"
                                required
                                rules={[
                                    {
                                        required: true,
                                        message: "Тема: обязательное поле!",
                                    },
                                    {
                                        type: "string",
                                        min: 10,
                                        message:
                                            "Тема: длина должна быть больше или равна 10 символам!",
                                    },
                                    {
                                        pattern: /^[?!,.а-яА-ЯёЁ0-9\s]+$/,
                                        message: "Тема: неверный формат поля!",
                                    },
                                ]}
                            >
                                <Input
                            ...
                            <Form.Item
                                ...
                                rules={[
                                    {
                                        required: true,
                                        message: "Вопрос: обязательное поле!",
                                    },
                                    {
                                        type: "string",
                                        min: 10,
                                        message:
                                            "Вопрос: длина должна быть больше или равна 10 символам!",
                                    },
                                ]}
                            >
                                <TextArea
