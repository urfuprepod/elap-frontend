...
export const VideoBlock = (props: VideoBlockProps): JSX.Element => {
    ...
    const [isOpenVideoViewingModal, setIsOpenVideoViewingModal] =
        useState<boolean>(false);
    const [isOpenEditVideoModal, setIsOpenEditVideoModal] =
        useState<boolean>(false);
    const [isOpenDeleteVideoModal, setIsOpenDeleteVideoModal] =
        useState<boolean>(false);
    ...
    return (
        <div className="video-block" key={props.id}>
            ...
            <Flex justify="space-between" align="center">
                ...
                ).length && location.pathname === "/admin/videos" ? (
            ...
            <Modal
                ...
                onOk={() => {
                    ...
                    httpClient
                        .axios()
                        .put(
                            config.endPoints.editVideo.replace(
                                "{videoId}",
                                props.id.toString() as string
                            ),
                            {
                                title: editVideo?.title,
                                videoPreviewImgUrl:
                                    editVideo?.videoPreviewImgUrl,
                                videoUrl: editVideo?.videoUrl,
                                videoDuration: editVideo?.videoDuration,
                            }
                        )
                        .then(() => {
            ...
            >
                <Form form={editVideoForm} layout="vertical">
                    ...
                    <Form.Item
                        ...
                        required
                        initialValue={props.videoPreviewImgUrl}
                        rules={[
                            {
                                required: true,
                                message:
                                    "Превью для видеосюжета: обязательное поле!",
                            },
                            {
                                type: "string",
                                min: 10,
                    ...
                    <Form.Item
                        ...
                        required
                        initialValue={props.videoDuration}
                        rules={[
                            {
                                required: true,
                                message:
                                    "Длительность видеосюжета: обязательное поле!",
                            },
                            {
                                pattern:
                    ...
                    <Form.Item
                        ...
                        required
                        initialValue={props.videoUrl}
                        rules={[
                            {
                                required: true,
                                message:
                                    "Ссылка на видеосюжет: обязательное поле!",
                            },
                            {
                                type: "string",
                                min: 10,
                            ...
                            () => ({
                                validator(_, value) {
                                    const isGoogleDrive = (
                                        value as string
                                    ).includes("drive.google.com/file/d");
            ...
            <Modal
                ...
                onOk={() => {
                    httpClient
                        .axios()
                        .delete(
                            config.endPoints.deleteVideo.replace(
                                "{videoId}",
                                props.id.toString() as string
                            )
                        )
